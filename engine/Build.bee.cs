...
class Build
{
    ...
    static void Main()
    {
        ...
        flutterRoot = "/Users/xingweizhu/WorkSpace/UnityApps/UIWidget/latest/dependencies/engine/src/";

        skiaRoot = "/Users/xingweizhu/WorkSpace/UnityApps/UIWidget/latest/dependencies/skia/";

        var libUIWidgets = Setup();
        var nativePrograms = new List<NativeProgram>();
        nativePrograms.Add(libUIWidgets);
        var xcodeProject = new XCodeProjectFile(nativePrograms, new NPath("libUIWidgets.xcodeproj/project.pbxproj"));
    }

    static NativeProgram Setup()
    {
        var np = new NativeProgram("libUIWidgets")
        {
            Sources =
        ...
        np.Sources.Add(iosSources);

        np.CompilerSettings().Add(c => c.WithCppLanguageVersion(CppLanguageVersion.Cpp17));
        np.CompilerSettings().Add(c => IsIosOrTvos(c), c => c.WithCustomFlags(new []{"-Wno-c++11-narrowing"}));

        //bitcode should be enabled since it is enabled by default in a common Unity built Xcode project
        ...
        np.Defines.Add("UIWIDGETS_ENGINE_VERSION=\\\"0.0\\\"", "SKIA_VERSION=\\\"0.0\\\"");
        np.Defines.Add(c => IsIosOrTvos(c), "UIWIDGETS_FORCE_ALIGNAS_8=\\\"1\\\"");
        ...
        //use local Ios toolChain since there is no newer build-in sdks than 10.1 in stevedore
        //ios 10.1 sdk can be used by this line: "var toolchain = ToolChain.Store.Ios().Sdk_10_1().Arm64();"
        //however it won't support c++17
        ...
        foreach (var codegen in codegens)
            {
                var config = new NativeProgramConfiguration(codegen, toolchain, lump: true);
                validConfigurations.Add(config);
                var builtNP = np.SetupSpecificConfiguration(config, toolchain.StaticLibraryFormat)
                    .DeployTo("build");
            }
    ...
    static void SetupTxt(NativeProgram np)
    {
        ...
        txtLib.CompilerSettings().Add(c => c.WithCustomFlags(new []{"-Wno-c++11-narrowing"}));
        ...
        sources = sources.Select(p => txtPath.Combine(p));
        txtLib.Sources.Add(sources);
        txtLib.Sources.Add(txtPath.Combine(new NPath("src/txt/platform_mac.mm")));
        txtLib.NonLumpableFiles.Add(sources);
