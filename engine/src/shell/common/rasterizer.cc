...
Rasterizer::Rasterizer(Delegate& delegate, TaskRunners task_runners)
    : Rasterizer(
          delegate, std::move(task_runners),
...
LayerTree* Rasterizer::GetLastLayerTree() { return last_layer_tree_.get(); }
...
void Rasterizer::Draw(fml::RefPtr<Pipeline<LayerTree>> pipeline) {
  TRACE_EVENT0("uiwidgets", "GPURasterizer::Draw");
...
sk_sp<SkImage> Rasterizer::DoMakeRasterSnapshot(
  ...
  {
    TRACE_EVENT0("uiwidgets", "MakeDeviceSnpashot");
...
RasterStatus Rasterizer::DrawToSurface(LayerTree& layer_tree) {
  TRACE_EVENT0("uiwidgets", "Rasterizer::DrawToSurface");
  FML_DCHECK(surface_);
...
static sk_sp<SkData> ScreenshotLayerTreeAsPicture(
    LayerTree* tree, CompositorContext& compositor_context) {
...
static sk_sp<SkData> ScreenshotLayerTreeAsImage(
    LayerTree* tree, CompositorContext& compositor_context,
    GrContext* surface_context, bool compressed) {
