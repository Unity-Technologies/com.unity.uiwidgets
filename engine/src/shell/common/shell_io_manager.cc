...
sk_sp<GrContext> ShellIOManager::CreateCompatibleResourceLoadingContext(
    GrBackend backend, sk_sp<const GrGLInterface> gl_interface) {
...
ShellIOManager::ShellIOManager(
    ...
    : resource_context_(std::move(resource_context)),
      ...
      unref_queue_(fml::MakeRefCounted<SkiaUnrefQueue>(
          ...
          fml::TimeDelta::FromMilliseconds(8), GetResourceContext())),
      weak_factory_(this),
...
// |IOManager|
fml::RefPtr<SkiaUnrefQueue> ShellIOManager::GetSkiaUnrefQueue() const {
  return unref_queue_;
}
