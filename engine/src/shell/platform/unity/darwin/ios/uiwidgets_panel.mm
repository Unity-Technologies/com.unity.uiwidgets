...
UIWIDGETS_API(float)
UIWidgetsPanel_onTestAdd(float a, float b) {
  return a + b;
}

UIWIDGETS_API(void)
UIWidgetsPanel_onTestClick() {
  auto* system_ptr = UIWidgetsSystem::GetInstancePtr();
  auto* interfaces = system_ptr->GetUnityInterfaces();
}
